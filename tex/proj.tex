...
abuse of notation and assume that each step in a choreography is
annotated with unique labels (which will be used by the
...
\begin{definition}[Projection, CTMC]\label{def:projCTMC} Given an
  ...
  \begin{displaymath}\small
        \begin{array}{lr}
          ...
          \proj (\role q,\ifTEl {E}a{p}{C_1}{C_2}, \iota) = 
          &  \boxed{\text{if }\role q=\role p}\\[2mm]
          \qquad\left\{ 
          \begin{array}{lll}
    ...
    variables $s_{\role q}$ (for each module in a choreography) used
    by the projection. Moreover, we say that a choreography is {\em
