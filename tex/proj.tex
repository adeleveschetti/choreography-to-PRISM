...
PRISM, we need to use labels on which each module in the interaction
can synchronise. Therefore, without loss of generality, we make a
slight abuse of notation and assume that each step in a choreography
is annotated with unique labels (which will be used by the
...
\begin{definition}[Projection, CTMC]\label{def:projCTMC} Given an
  ...
  \begin{displaymath}\small
        \begin{array}{lr}
          ...
          \proj (\role q,\interact{p}{\role p_1,\ldots,\role p_n}, \iota)=\ \proj (\role{q}, C_1, \iota)
