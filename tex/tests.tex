...
We focus on three benchmarks: the dice program that we compare with
the test cases reported in the PRISM
...
For lack of space, we do not report the generated PRISM files, which
can be found in the online repository \cite{repository}.

\subsection{The Dice Program}
% The first test case we focus on is the Dice Program \cite{KY76}.  With
Although choreographies are meant to abstract concurrent interactions,
they can also be used to model single-process systems~\cite{M23}, such
as the Dice Program~\cite{KY76}.
% The Dice Program~\cite{KY76} example shows that choreographies can
% model single-process systems.
%
% The following program 
This example simulates a die using only unbiased coins. Starting from
the initial vertex (state $s_0$), the process involves the iterative
flipping of a coin. Upon obtaining heads, the upper branch is chosen,
and in the case of tails, the lower branch is taken. This sequential
process persists until the final determination of the die's value.  In
particular, the PRISM model uses two variables:
\begin{itemize}
\item \codeprism{d} represents the value of the dice, ranging from 0
  to 6 (initially set to 0).
\item \codeprism{STATE} is the state variable, ranging from 0 to 7
  (initially set to 0).
\end{itemize}
 ...
 \begin{lstlisting}[style=chor-color,caption={Choreography for the Dice Program},captionpos=b,label={ex1-code}]
	...
	\end{lstlisting}
%
        The PRISM model produced by our method closely aligns with the
        original model with only minimal details that differ between
        the two.  Additionally, to ensure the generated program's
        accuracy, we evaluated whether the probability of reaching a
        state where the dice displays \texttt{d=k} for every possible
        \texttt{k} from 1 to 6 equals 1/6. Our simulations show that
        the probabilities calculated by our generated model fully
        matched those from the original PRISM model.
...
%Specifically, in Figure \ref{ex3-res}, we present the probability of block creation. Discrepancies between the results of the original model and the generated one arise from the nature of this probability calculation, which involves simulation and may therefore vary with each run.
