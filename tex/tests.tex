...
\subsection{The Dice Program}
 ...
 \begin{itemize}
	\item \codeprism{d}: This variable represents the state of the system, ranging from 0 to 7. Initially, it is set to 0.
	\item \codeprism{STATE}: This variable represents the value of the die, ranging from 0 to 6. Initially, it is set to 0.
 \end{itemize}
%We briefly explain the overall process without providing the full PRISM code. For instance, the command 
%\codeprism{[] (STATE=0) -> 0.5 : (STATE'=1) + 0.5 : (STATE'=2);}
%means that when the system is in state 0 (i.e., the die hasn't been rolled yet), there is a 50\% chance it will transition to state 1 (die rolls a 1) and a 50\% chance it will transition to state 2 (die rolls a 2).
%Other transition blocks follow similar logic for states 1 to 6.
%State 7 represents an absorbing state where the die has come to rest after being rolled. Transitions from state 7 always lead back to itself.
In Listing \ref{ex1-code}, we provide part of the model in our choreographic language. 
...
we evaluated whether the probability of reaching a state where the die displays \texttt{d=k} for every possible \texttt{k} from 1 to 6 equals 1/6. This reflects the expected behavior of a fair die. In particular, the probabilities calculated by our generated model fully matched those from the original PRISM model. 
...
\end{wrapfigure} 
\end{comment}
In \cite{DBLP:journals/concurrency/BistarelliNGLMV23}, the authors extended the PRISM model checker to incorporate dynamic data types, enhancing its capabilities to model the Proof of Work protocol used in the Bitcoin blockchain \cite{bitcoin}.
...
\end{wrapfigure} 
\end{comment}
We now present the Hybrid Casper Protocol as modeled in PRISM, as detailed in \cite{DBLP:journals/distribledger/GallettaLMV23}. The Hybrid Casper protocol represents a hybrid consensus protocol for blockchains, merging features from both Proof of Work and Proof of Stake protocols. 

The modeling approach is very similar to the one used for the Proof of Work Bitcoin protocol. Specifically, the Hybrid Casper protocol is represented in PRISM as the parallel composition of $n$ \texttt{Validator} modules, along with the modules \texttt{Vote\_Manager} and \texttt{Network}. The \texttt{Validator} module closely resembles the \texttt{Miner} module from the preceding protocol. The module \texttt{Vote\_Manager} is responsible for storing tables containing votes for each block and computing associated rewards/penalties.
...
This difference is due to the fact that certain commands that could be combined, but our generation lacks the automatic capability to perform this check. While the results exhibit similarity, running simulations for the generated model takes PRISM 39.016 seconds, compared to the 22.051 seconds required for the original model.
