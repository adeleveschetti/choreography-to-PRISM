...
\begin{displaymath}\small
  ...
  \end{array}
\end{displaymath}
Above, $S(E_j)$ is an unspecified (terminating) evaluation of the
expression $E$ under state $S$. The transition relation is a Discrete
Time Markov Chain (DTMC) or a Continuous Time Markov Chain (CTMC)
depending on whether we use probabilities or rates in the branching
construct~\cite{ctmc}. States of the Markov chain are the pairs
$(S,C)$.
%
