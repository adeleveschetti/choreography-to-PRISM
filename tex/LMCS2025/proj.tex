...
\begin{example}\label{example-proj}
  ...
  \begin{displaymath}
    \begin{array}{lll}
      C \stackrel{\textcolor{sh_keyword}{\mathsf{def}}}{=} \interactBasel{p}a{q}
      \left\{
      \begin{array}{lll}
        \lambda_1: (x'=1)\&(y'=2);\ C
        \\
...
\begin{definition}[Head Modules]
  ...
  \begin{displaymath}
    \begin{array}{rll}
      ...
      & = \ \{\nr, \nr_1,\dots,\nr_n\}\\
      \hmodules(\ifTE {E}{\nr}{C_1}{C_2}) \ & =\ \{\nr\}\\
      \hmodules(X) \ & =\ \hmodules (C)\qquad\qquad(\text{if } X\stackrel{\textcolor{sh_keyword}{\mathsf{def}}}{=} C\in\defin)\\
...
\begin{definition}[Strongly Connected Choreography]
  ...
  \begin{displaymath}
    \begin{array}{c}
      ...
      \sconnected(C)\qquad\qquad X\stackrel{\textcolor{sh_keyword}{\mathsf{def}}}{=} C\in\defin
...
\begin{displaymath}\small
  X\stackrel{\textcolor{sh_keyword}{\mathsf{def}}}{=} 
  \interactBase{p}{q}:\,
  \left(
    \begin{array}{l}
...
state $S$ with the additional variables $s_{\role q}$ (one for each
role $\role q$), introduced by the projection.  The projection uses
alphabetised parallel composition~$\pp$, where modules synchronise
only on shared labels.
...
\begin{theorem}[Projection]\label{thm:epp}
  Let $C$ be a choreography such that $\sconnected(C)$, and let $S_+$
  be the extension of a global state $S$ with the control variables
  ...
  \[
    ...
    S_+ \red{\lambda} S_+' ,
  \]
  and moreover, for every role $\role q$,
  \[
