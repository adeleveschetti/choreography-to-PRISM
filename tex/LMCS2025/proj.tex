...
begin with choreographies that involve rates: \marco{write about $a$ vs $a_j$}
% 
\begin{definition}[Projection, CTMC]\label{def:projCTMC} Given an
  ...
  \begin{displaymath}\small
        \begin{array}{lr}
          ...
          \proj (\role q,\interactl{p}{a}{\role p_1,\ldots,\role p_n})= 
          &  \boxed{\text{if }\role q=\role p}\\[2mm]
          \qquad
          % \{ 
          \left\{\commandBase {a_j} {\ s_{q}\!=\! a} {\lambda_j:}
          \ s_{q}'\!=\! \mathsf{top}_{\role q}(C_j)\ 
          \&\ \projE {u_j}q\right\}_{j\in J}
          \\
          \qquad\cup\ \bigcup_{j} \proj (\role q, C_j)
...
\begin{theorem}[Projection]\label{thm:epp}
  ...
  \marco{Here we need to relate $S'_+$ with $C'$. Discuss an example
    with Adele to see how it looks}
\end{theorem}
\begin{proof}
