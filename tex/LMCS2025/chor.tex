...
\item {\em For each construct.} $\textsf{foreach } e(i)\ u@A[i];\,
  C$. This construct allows to...
  
\item {\em Non-deterministic Synchronisation.} Our implementation uses
  the language construct $\allsynch pGiI$ where $G$ has the form
  $\chorcommand g\lambda u$. The intuition behind this operation is
  that the set of roles $\role p_i$ synchronises but each of them can
  non-deterministically pick from a set of different local actions. In
  order to clarify this further, consider the following synchronised
  interactions between roles $\role p$ and $\role q$:
\end{itemize}

  

\begin{example}\label{example3}
  The following definition describes a synchronized interaction
  between roles $\role p$ and $q$:
  \begin{displaymath}
    \begin{array}{lll}
