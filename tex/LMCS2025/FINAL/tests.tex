...
modules. This implementation realises the projection formally defined
in Section~\ref{sec:proj} and ensures that each benchmark presented
...
our formal framework: it guarantees that the semantics of the
...
equivalent behaviour on both the projected and original models}.
\newmark{R2.18/R3.3}%
...
\cite{repository}.
\newmark{R2.19}%
\new{For the first four benchmarks, PRISM performs exact
...
summarises the number of lines of code (LoC) taken from the listings
shown in this article. The measurements reflect only the fragments
printed in the paper and exclude all variable declarations in both
the choreography and the corresponding PRISM models. Moreover,
complete PRISM models are typically larger, as each interaction from
the global choreography expands into multiple module-specific
commands.}
