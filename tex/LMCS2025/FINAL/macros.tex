...
% ---- Highlighting of new text in revision 2 --------------------------
% Toggle: set \highlightnewtrue to show additions in blue + margin IDs,
%         \highlightnewfalse to render everything as normal black text.
\newif\ifhighlightnew
\highlightnewfalse
% Plain highlight (no margin marker)
\newcommand{\new}[1]{\ifhighlightnew\textcolor{blue}{#1}\else#1\fi}
% Margin marker only, no text wrapping.
% Usage: place \newmark{R2.1} at the start of a paragraph or near a change.
\newcommand{\newmark}[1]{%
  \ifhighlightnew\marginpar{\scriptsize\color{blue}\textbf{[#1]}}\fi
}
% Highlight with a margin marker carrying a reviewer-comment ID.
% Usage: \newid{R3.2}{the new text...}
\newcommand{\newid}[2]{%
  \ifhighlightnew
    \marginpar{\scriptsize\color{blue}\textbf{[#1]}}%
    \textcolor{blue}{#2}%
  \else
    #2%
  \fi
...
\newcommand{\interactl}[3]{\interactBasel{#1}{#2}{#3}:\,\Sigma_{j\in J}\lambda_j: u_j;\ \anno{C}_j}
