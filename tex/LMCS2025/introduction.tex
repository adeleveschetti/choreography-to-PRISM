...
{
In the kinds of distributed applications we can apply our technique on, the behaviour of the
system is largely determined by how components communicate, rather than by
arbitrary interleavings of independent local steps.  What matters is which
participants interact, in which order, and under which conditions, and this
is exactly the information that a choreography makes explicit.  Our setting
is therefore designed towards systems whose main dynamics can be described in
terms of such structured interaction patterns, while more loosely coordinated
or highly unstructured behaviours are deliberately not addressed by the
framework.
}

This paper presents a choreographic language designed for modelling
concurrent probabilistic systems.
%\textcolor{red}{Our language enhances the modelling of concurrent probabilistic systems, providing clarity often absent in 
...
%
The behaviour described by the choreography could be represented in
PRISM as:
%
\[
\begin{array}{llll}
\role{p}: \{ & \quad \commandBase {a}{s_{\role p}=0} {\lambda_1:(x'=1)\ \&\ (s_{\role p}'=1)} \quad\qquad \commandBase {}{s_{\role p}=1} {1:(s_{\role p}'=0)}\\[2mm]
              & \quad \commandBase {b}{s_{\role p}=0} {\lambda_2:(x'=3)\ \&\ (s_{\role p}'=2)} \quad\qquad \commandBase {}{s_{\role p}=2} {1:(s_{\role p}'=0)} \quad \}\\[2mm]
...
is a list of commands for modifying the state. In this particular
translation, labels $a$ and $b$ are used for simulating the two
branches of the choreography, respectively.
%
...
counters \( s_{\role p} \) and \( s_{\role q} \) to 0 due to the
