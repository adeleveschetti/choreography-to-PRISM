Understanding and programming distributed systems pose difficult
...
This work presents a choreographic language designed for modelling
...
\mypar{Contributions.} Our contributions can be categorised as:
\begin{itemize} 
\item We propose a choreographic language equipped with well-defined
  ...
  concurrent systems with probabilistic behaviours. To the best of our
  knowledge, this is the first probabilistic choreography language
  that is not a type abstraction.
\item Then, we introduce a semantics for PRISM, based on its original
  semantics~\cite{PRISMdoc}.
\item Furthermore, we establish a rigorous definition for a
  translation function from choreographies to PRISM, and address its
  correctness. This translation (projection) serves as a crucial
  intermediary step in transforming (compiling) models described in
  our choreographic language into PRISM-compatible representations.
