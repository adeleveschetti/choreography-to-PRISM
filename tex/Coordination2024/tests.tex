...
\vspace{-0.05cm}
 \begin{lstlisting}[style=chor-color,caption={Choreography for the Dice Program},captionpos=b,label={ex1-code}]
   ...
   { 
   C $\coloneqq$ Server $\rightarrow$ Station[i] :
         (+["gamma*1"] "(s=[i])&(a=0)""(s[i]=0)"$\rightarrow$"(s'=s+1)""(s[i]'=0)" ; C 
          +["gamma*lambda"] "(s=[i])&(a=0)" "(s[i]=0)" $\rightarrow$ "(a'=1)" "(s[i]'=1)" ;
               Server $\rightarrow$ Station[i] : 
                  (["mu*1"] "(s=[i])&(a=1)" "(s[i]=1)" $\rightarrow$ "(s'=s+1)&(a'=0)" "(s[i]'=0)";C ))
   }
      \end{lstlisting}
