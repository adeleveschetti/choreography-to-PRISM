...
The protocol aims to manage exclusive access to a single file, controlled by the \codeprism{CheckOut} process. Users can request access, which can be granted or dbased on the file's current status. The goal is to ensure that only one user possesses the file at any 
