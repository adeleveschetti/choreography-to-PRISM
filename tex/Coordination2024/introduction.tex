...
\textcolor{red}{Choreographies excel in environments characterized by deterministic communication patterns and clear sequencing of actions. However, they may not be well-suited for managing highly unpredictable or dynamically changing interactions. In scenarios where processes exhibit significant variability or intricate coordination logic, choreographies may lack the flexibility or adaptability required for effective orchestration.}
...
\item Then, we introduce a semantics for PRISM (\S~\ref{sec:prism}),
  ...
  \marco{COMMENT ON THIS: . In item (2), I would clearly state that/if
    the formal semantics of PRISM is faithul to the original one (I
    assume so) and that the purpose is to present the minimal fragment
    of PRISM that is needed for PRISM code generation from
    choreographies.}
...
\item \textcolor{red}{Lastly, our implementation involves a compiler that executes the defined translation process. This compiler transforms choreographies into PRISM models, enabling users to utilize PRISM's robust analysis features while benefiting from the expressiveness of our choreographic language.}
\end{itemize}
