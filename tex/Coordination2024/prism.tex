...
\begin{example}
  \marco{This needs to be fixed, it misses recursion and it doesn't
    work with probabilities} The choreography presented in Example
  \ref{example2} can be described by the following PRISM network $M$:
\begin{displaymath}
  \begin{array}{llll}
    \role{p}: \{ & \quad \commandBase {a}{s_p=0} {\mu_1:(x'=1)}\\
                 & \quad \commandBase {b}{s_p=0} {\mu_2:(x'=3)}\quad \}\\[2mm]
    \role{q}: \{ & \quad \commandBase {a}{s_q=0} {\gamma_1:(y'=2)}\\
                 & \quad \commandBase {b}{s_q=0} {\gamma_2:(y'=1)}\quad \}\\
  \end{array}
\end{displaymath}
...
\begin{displaymath}\small
  \begin{array}{rlll}
    F_1=\commandBase {a}{s_p=0\ \&\ s_q=0} {\quad} \mu_1*\gamma_1 : (x'=1) \& (y'=2) \\
    F_2=\commandBase {a}{s_p=0\ \&\ s_q=0} {\quad} \mu_2*\gamma_2 : (x'=3) \& (y'=1) \\
